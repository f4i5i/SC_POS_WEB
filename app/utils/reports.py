...
def generate_daily_report(day_close, sales_list):
    ...
    info_data = [
        ['Business Name:', current_app.config.get('BUSINESS_NAME', 'Zaviyar Perfumes & Attars')],
    ...
    footer = Paragraph(
        f"Generated by Zaviyar Perfumes & Attars POS System on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}",
        footer_style
    )
    story.append(footer)
